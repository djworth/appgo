...
func (ctx Context) Redirect(url_ string) {
	http.Redirect(ctx.Response, ctx.Request, url_, http.StatusFound)
}
...
func (ctx Context) SetCookie(key string, val string, exp int64) {
	var utc time.Time
	if exp == 0 {
		utc = time.Unix(2147483647, 0).UTC( // year 2038
		)
	} else {
		utc = time.Unix(time.Now().UTC().Seconds()+exp, 0).UTC()
	}
...
func (u *UserType) GetLoginURL(url_ string) string {
	login, _ := user.LoginURL(u.Context, url_)
	return login
}

func (u *UserType) getLogoutURL(url_ string) string {
	logout, _ := user.LogoutURL(u.Context, url_)
	return logout
}
